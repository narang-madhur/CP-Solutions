...
void solve()
{
    ...
    //what if same numbers?
    // for(int i = 1 ; i < eq.size() ; i++)
    // {
    //     if((eq[i] == '0' || eq[i] == '1') && (eq[i - 1] == '0' || eq[i - 1] == '1'))
    //     {
    //         cout<<"NO"<<endl;
    //         return;
    //     }
    ...
    for(int i = 0 ; i < eq.size() ; i++)
    {
        ...
        {
            if(lastSortedIndex == numCount)
            {
                lastSortedIndex--;
            }
            
            if(lastUnsortedIndex.size() > 0 && lastUnsortedIndex.top() == numCount)
            {
                //must be prev value?
                lastUnsortedIndex.pop();
            }
        ...
        {
            if(numCount < 2 || lastSortedIndex == numCount)
            {
                cout<<"NO"<<endl;
                return;
            }
            ...
            if(lastUnsortedIndex.size()== 0 || lastUnsortedIndex.top() != numCount)
            {
                lastUnsortedIndex.push(numCount);
            }
        }
        else if(eq[i] == '1')
        {
