...
/*------------------------------------begin------------------------------------*/
double solve2(int i , int required , int &question)
{
    if(i == question)
    {
    ...
    double ans = 0;
    ans += solve2(i + 1 , required + 1 , question);
    ans += solve2(i + 1 , required - 1 , question);
...
void solve()
{
    ...
    {
        if(original[i] == '+')originalAns++;
        else originalAns--;
    }

    string received;
    cin >> received;
    int recievedAns = 0 , question = 0;
    for(int i = 0 ; i < received.size() ; i++)
    {
        if(received[i] == '+') recievedAns++;
        else if(received[i] == '-') recievedAns--;
        else question++;
    }
    ...
    {
        int required = recievedAns - originalAns;
        double ans = solve2(0 , required , question);  
        double power = pow(2 , question);
