...
void solve()
{
    ...
    for(int i = 1 ; i < n ; i++)
    {
        if(prefixSum[i] - prefixSum[i - 1] > n)
        {
            twoNums.push_back(prefixSum[i - 1]);
            twoNums.push_back(prefixSum[i]);
        }
        else if(nums.count(prefixSum[i] - prefixSum[i - 1]) == 0)
        {
            twoNums.push_back(prefixSum[i - 1]);
            twoNums.push_back(prefixSum[i]);
        }
        ...
        {
            nums.erase(prefixSum[i] - prefixSum[i - 1]);
        }
    }
    ...
    else if(twoNums.size() == 2 && nums.size() == 2)
    {
        if(twoNums[1] - twoNums[0] == *nums.begin() + *(++nums.begin()))
        {
            ans = true;
        }
    }
