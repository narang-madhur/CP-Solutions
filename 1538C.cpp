...
/*------------------------------------begin------------------------------------*/
long long solve2(vector<int> &nums , int limit)
{
    long long ans = 0;

    for(int i = 0 ; i < nums.size() ; i++)
    {
        int updatedLimit = limit - nums[i];

        int ind = upper_bound(nums.begin() , nums.end() , updatedLimit) - nums.begin() - 1;
        if(i <= ind)
        {
            ind--;
        }
...
void solve()
{
    ...
    long long rAns = solve2(nums , r);
    long long lAns = solve2(nums , l - 1);
    //cout<<rAns<<" "<<lAns<<endl;
