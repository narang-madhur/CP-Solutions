...
void solve()
{
    ...
    if(n % 2 == 1)
    {
        ...
        for(int i = 1 ; i < n ; i += 2)
        {
            int reqFloors = max(heights[i + 1] , heights[i - 1]) + 1 - heights[i];
            if(reqFloors < 0) reqFloors = 0;
    ...
    vector<int> prevAlternatingSum(n + 2 , 0);
    vector<int> nextAlternatingSum(n + 2 , 0);
    ...
    for(int i = 1 ; i < n - 1 ; i++)
    {
        ...
        if(i - 2 >= 0) prevAlternatingSum[i] += prevAlternatingSum[i - 2];
        prevAlternatingSum[i] += reqFloors;
    }
    ...
    for(int i = n - 2 ; i > 0 ; i--)
    {
        ...
        if(i + 2 < n) nextAlternatingSum[i] += nextAlternatingSum[i + 2];
        nextAlternatingSum[i] += reqFloors;
    }
    ...
    for(int i = 1 ; i < n - 1 ; i++)
    {
        if(i % 2 == 1)
        {
            minAns = min(minAns , min(nextAlternatingSum[i + 2] , nextAlternatingSum[i + 3]) + prevAlternatingSum[i]);
        }
        else
        {
            minAns = min(minAns , nextAlternatingSum[i + 2] + prevAlternatingSum[i]);
        }
    }
