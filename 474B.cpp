...
void solve()
{
    ...
    vector<int> pileSizes(n);
    for(auto &x : pileSizes) cin >> x;
    for(int i = 1; i < n; i++)
    {
        pileSizes[i] += pileSizes[i - 1];
    }
    ...
    for(auto &x : labels)
    {
        ...
        auto ans = lower_bound(pileSizes.begin() , pileSizes.end() , x);
        if(ans == pileSizes.end())
        {
            ans--;
            cout<<ans - pileSizes.begin() + 1<<endl;
        }
        else
        {
            cout<<ans - pileSizes.begin() + 1<<endl;
