...
/*------------------------------------begin------------------------------------*/
vector<int> solve2(int i , vector<vector<int>> &adj , string &colors)
{
    vector<int> smallAns(3 , 0);
    if(colors[i] == 'B') smallAns[0]++;
    else smallAns[1]++;

    for(int j = 0 ; j < adj[i].size() ; j++)
    {
        vector<int> childAns = solve2(adj[i][j] , adj , colors);
        smallAns = {smallAns[0] + childAns[0] , smallAns[1] + childAns[1] , smallAns[2] + childAns[2]};
    }

    if(smallAns[0] == smallAns[1]) smallAns[2]++;
...
void solve()
{
    ...
    cout << solve2(0 , adj , colors)[2] << endl;
