...
/*------------------------------------begin------------------------------------*/
unsigned long long getRemaining(long long l , long long r)
{
    unsigned long long ans = 0;

    unsigned long long lRoot = sqrtl(l);
    
    unsigned long long curAns = (unsigned long long)(r / lRoot) - (unsigned long long)((l - 1) / lRoot);
    ans += curAns;
...
void solve()
{
    ...
    unsigned long long lRoot = sqrtl(l);
    if(lRoot * lRoot != l) lRoot++;

    unsigned long long rRoot = sqrtl(r);

    ans += (rRoot - lRoot) * 3;

    unsigned long long sqrLRoot = lRoot * lRoot;
    ans += getRemaining(l , sqrLRoot - 1);

    unsigned long long sqrRRoot = rRoot * rRoot;
    ans += getRemaining(sqrRRoot , r);
