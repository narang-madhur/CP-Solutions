...
void solve()
{
    ...
    reverse(binaryX.begin(), binaryX.end());
    binaryX += '0';
    ...
    for(int i = binaryX.size() - 1 ; i >= 0 ; i--)
    {
        if(binaryX[i] == '1')
        {
            if((i + 2) >= binaryX.size() || ((i + 1) < binaryX.size() && binaryX[i + 1] == '1'))
            {
                cout << -1 << endl;
                return;
            }
    ...
    {
        if(firstNum[i] == '1') firstAns += pow(2, firstNum.size() - i - 1);

        if(secondNum[i] == '1') secondAns += pow(2, secondNum.size() - i - 1);
    }
