...
void solve()
{
    ...
    int ans = INT_MAX;
    for(auto &pairs : indexMap)
    {
        int maxBlock = 0;
        pairs.second.insert(pairs.second.begin() , -1);
        pairs.second.push_back(s.size());

        for(int l = 1 ; l < pairs.second.size() ; l++)
        {
            maxBlock = max(maxBlock , (pairs.second[l] - pairs.second[l - 1] - 1));
        }
