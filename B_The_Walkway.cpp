...
void solve()
{
    ...
    int minAns = totalCount + m;
    int minFreq = 0;

    for(int i = 1 ; i < s.size() - 1 ; i++)
    {
        int curRemoval = count[i] + count[i + 1];
        int curAddition = (s[i + 1] - s[i - 1] - 1) / d;
        int curAns = totalCount - curRemoval +  curAddition + m - 1;
        if(s[i] == 1) curAns++;
        ...
        else if(minAns == curAns)
        {
            minFreq++;
        }
    }

    cout<<minAns<<" "<<minFreq<<endl;
