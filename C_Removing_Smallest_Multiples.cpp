...
void solve()
{
    ...
    unordered_set<int> notThere;
    vector<int> notThere2;
    for(int i = 0 ; i < n ; i++) 
    {
        if(str[i] == '0') 
        {
            notThere.insert(i + 1);
            notThere2.push_back(i + 1);
        }
    }

    int cost = 0;
    unordered_set<int> removed;
    for(int i = 0 ; i < notThere2.size() ; i++)
    {
        for(int j = notThere2[i] ; j <= n ; j += notThere2[i])
        {
            if(notThere.count(j) == 1)
            {
                // cout<<notThere2[i] <<" * ";
                cost += notThere2[i];
                removed.insert(j);
