...
long long getLower(long long curJump , vector<long long> &maxJump , vector<long long> &heightSum)
{
    int l = 0;
    int r = maxJump.size() - 1;
    ...
    while (l <= r)
    {
        ...
        if(maxJump[mid] <= curJump)
        {
            answer = heightSum[mid];
            l= mid + 1;
        }
        else
        {
...
void solve()
{
    ...
    vector<long long> heightSum(n);
    vector<long long> maxJump(n);
    ...
    for(int i = 1 ; i < n ; i++)
    {
        maxJump[i] = max(maxJump[i - 1] , jumps[i]);
        heightSum[i] += heightSum[i - 1] + jumps[i];
    }
    ...
    for(int i = 0 ; i < q ; i++)
    {
        long long curJump;
        cin >> curJump;
        cout<<getLower(curJump, maxJump , heightSum)<<" ";
    }
