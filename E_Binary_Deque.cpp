...
void solve()
{
    ...
    unordered_map<int , int> frontSum(n);
    frontSum[0] = -1;
    sum = 0;
    for(int i = 0 ; i < n ; i++)
    {
    ...
    unordered_map<int , int> backSum(n);
    backSum[0] = n;
    sum = 0;
    for(int i = n - 1 ; i >= 0 ; i--)
    {
    ...
    int minCost = INT_MAX;
    for(auto &pairs : frontSum)
    {
        if(backSum.count(removedSum - pairs.first) == 1)
        {
            int curCost = n - backSum[removedSum - pairs.first] + pairs.second + 1;
