...
void solve()
{
    ...
    for(int i = 0 ; i < m ; i++)
    {
        ...
        if(stranger[a] == -1) stranger[a] = b;
        else stranger[a] = min(stranger[a] , b);
    }
    stranger[n] = n + 1;

    multiset<int> nextStranger;
    for(int i = 1 ; i <= n ; i++)
    {
        if(stranger[i] != -1)
        {
            nextStranger.insert(stranger[i]);
        }
    }
    ...
    for(int i = 1 ; i <= n ; i++)
    {
        ...
        {
            // cout << stranger[i] << " ";
            auto it = nextStranger.upper_bound(i);
            ans += (min(stranger[i] , *it) - i);
            nextStranger.erase(nextStranger.find(stranger[i]));
        }
        else
        {
            auto it = nextStranger.upper_bound(i);
