...
int solve(int n , int &a , int &b , int &c , vector<int> &dp)
{
    ...
    if(n < min(a , min(b , c))) return INT_MIN;
    ...
    if(n >= a)
    {
        smallAns = max(smallAns , solve(n - a , a , b , c , dp) + 1);
    }
    if(n >= b)
    {
        smallAns = max(smallAns , solve(n - b , a , b , c , dp) + 1);
    }
    if(n >= c)
    {
        smallAns = max(smallAns , solve(n - c , a , b , c , dp) + 1);
    }
