...
void solve()
{
    ...
    vector<int> a(n + 1 , 0) , x(m + 1 , 0) , y(m + 1, 0) , degree(n + 1 , 0);
    for(int i = 1 ; i <= n ; i++)
    {
        cin >> a[i];
    }
    ...
    int ans = INT_MAX;
    for(int i = 1 ; i <= n ; i++) if(degree[i] % 2 == 1) ans = min(ans , a[i]);

	for(int i = 1 ; i <= m ; i++) if(degree[x[i]] % 2 == 0 && degree[y[i]] % 2 == 0) ans = min(ans , a[x[i]] + a[y[i]]);
