...
void solve()
{
    ...
    map<int , int> differenceMapping;
    // differenceMapping[n - 1] = -1;
    int totalSize = n;
    ...
    for(int i = 0 ; i < c ; i++)
    {
        ...
        int curDiff = totalSize - l;
        totalSize += curSize;
        differenceMapping[totalSize - 1] = curDiff;
    }
    ...
    for(int i = 0 ; i < q ; i++)
    {
        ...
        while(curIndex >= n)
        {
            int curDiff = differenceMapping.lower_bound(curIndex)->second;
            curIndex -= curDiff;
        }

        cout << s[curIndex] << endl;
    }
