...
void solve()
{
    ...
    set<int> remaining1;
    for(int i = 1 ; i <= n ; i++) remaining1.insert(i);
    set<int> remaining2;
    for(int i = 1 ; i <= n ; i++) remaining2.insert(i);

    unordered_map<int , vector<pair<int , int>>> ans;
    for(int i = 1 ; i <= n ; i++)
    {
        if(remaining1.count(a[i]) == 1 && *remaining2.begin() <= a[i])
        {
            ans[a[i]].push_back({a[i] , *remaining2.begin()});
            remaining1.erase(a[i]);
            remaining2.erase(*remaining2.begin());
        }
        else if(remaining2.count(a[i]) == 1 && *remaining1.begin() <= a[i])
        {
            ans[a[i]].push_back({*remaining1.begin() , a[i]});
            remaining2.erase(a[i]);
    ...
    {
        p.push_back(ans[tempA[i]].back().first);
        q.push_back(ans[tempA[i]].back().second);
        ans[tempA[i]].pop_back();
    }
