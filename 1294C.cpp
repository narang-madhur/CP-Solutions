...
void solve()
{
    ...
    for(auto pairs : div)
    {
        ans.insert(pairs.first);
        pairs.second--;

        if(pairs.second >= 2)
        {
            pairs.second -= 2;
            ans.insert(pairs.first * pairs.first);
        }

        if(pairs.second >= 3)
        {
            ans.insert(pairs.first * pairs.first * pairs.first);
            pairs.second -= 3;
        }
    }
    ...
    if(ans.size() >= 2)
    {
        int curMul = *ans.begin() *  *(++ans.begin());
        int leftNum = tempN / curMul;
        if(leftNum == *ans.begin() || leftNum == *(++ans.begin()) || leftNum == 1) 
        {
            cout<<"NO"<<endl;
            return;
        }
        cout<<"YES"<<endl;
        cout<<*ans.begin()<<" "<<*(++ans.begin())<<" "<<(tempN / curMul)<<endl;
    }
    else 
    {
        cout<<"NO"<<endl; 
