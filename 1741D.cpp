...
/*------------------------------------begin------------------------------------*/
int ans = 0;
pair<int , int> solveFurther(vector<int> &nums , int l , int r)
{
    if(l == r)
    {
    ...
    pair<int , int> leftAns = solveFurther(nums , l , mid);
    pair<int , int> rightAns = solveFurther(nums , mid + 1 , r);

    if(rightAns.first == -1 || leftAns.first == -1 || (leftAns.second - leftAns.first + 1) != (r - l + 1) / 2 || (rightAns.second - rightAns.first + 1) != (r - l + 1) / 2)
    {
        return {-1 , -1};
    }
...
void solve()
{
    ...
    ans = 0;
    pair<int , int> ansPair = solveFurther(nums , 0 , n - 1);
    if(ansPair.first != -1) cout << ans << endl;
    else cout << -1 << endl;
