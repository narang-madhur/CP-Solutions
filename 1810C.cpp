...
void solve()
{
    ...
    long long remainingCost = LONG_LONG_MAX;
    long long curCost = 0;
    int curNum = 1;
    for(int i = 0 ; i < nums.size() ; i++)
    {
        if(nums[i] != curNum)
        {
            if(curNum == 1)
            {
                remainingCost = min(remainingCost , a + curCost + ((nums.size() - i) * r));
            }
            else
            {
                remainingCost = min(remainingCost , curCost + ((nums.size() - i) * r));
            }

            curCost += (nums[i] - curNum) * a;
            curNum = nums[i];
        }

        curNum++;
    }
    remainingCost = min(remainingCost , curCost);

    cout<<cost + remainingCost << endl;
