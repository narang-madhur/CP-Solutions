...
#define sz size()
#define optimalCode ios_base::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL)
...
void solve(vector<pair<bool , int>> &is_prime)
{
    int l , r;
    cin>> l >> r;
    ...
    if(l != r)
    {
        ...
        if(secondNum >= 2 && secondNum + 2 >= l && secondNum + 2 <= r)
        {
            cout<<2<<" "<<secondNum<<endl;
            return;
        }
        else
        {
    ...
    else
    {
        ...
        {
            int num1 = is_prime[l].second;
            int num2 = l - is_prime[l].second;
            if(num2 >= 2 && num1 + num2 >= l && num1 + num2 <= r)
            {
                cout<<num1<<" "<<num2<<endl;
                return;
            }
...
signed main()
{
    ...
    int n = 10000000;
    vector<pair<bool , int>> is_prime(n+1, {true , 1});
    is_prime[0] = is_prime[1] = {false , 0};
    for (int i = 2; i * i <= n; i++) {
        if (is_prime[i].first  == true) {
