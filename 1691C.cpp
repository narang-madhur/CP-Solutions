...
void solve()
{
    ...
    if(onePos.size() > 0)
    {
        if(n - 1 - onePos[onePos.size() - 1] <= k)
        {
            swap(s[onePos[onePos.size() - 1]] , s[n - 1]);
            k -= (n - 1 - onePos[onePos.size() - 1]);

            onePos[onePos.size() - 1] = n - 1;
        }

        if(onePos[0] != (n - 1) && onePos[0] - 0 <= k)
        {
            swap(s[onePos[0]] , s[0]);
            k -= (onePos[0] - 0);
        }
